...
def view_mainmenulist(name):
    ...
    return render_template("view_mainmenulist.html",  datas=data, datas2=new_dict.items(), name=res_name,
                           total=int(tot_count), limit=limit, page=page, page_count=int((tot_count/4)+1))

#임시
@application.route("/view_mainmenulist2/<name>/")
def view_mainmenulist2(name):
    data = DB.get_food_byname(str(name))
    tot_count=len(data)
    res_name=name
    ...
    for k,v in enumerate(data):
            new_dict[k]=v
    listdata=list(new_dict.items())[start_idx:end_index]
    
    
    print("##data:", data)
    
    return render_template("view_mainmenulist2.html",  datas=listdata, for_idx=range(0,3), listlen=len(listdata), datas2=new_dict.items(), name=res_name,
                           total=int(tot_count), limit=limit, page=page, page_count=int((tot_count/4)+1))
