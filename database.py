...
class DBhandler:
    ...
    #리뷰 데이터베이스 함수
    def insert_review(self, write, data, image_path):
        review_info={
            "restaurant_name": data['restaurant_name'],
            "year": data['year'],
            "month": data['month'],
    ...
    def get_maps(self):
        maps=self.db.child("restaurant").get().val()
        return maps
    ...
    def random_recommend(self):
        ...
        for res in restaurants.each():
            value=res.val();
            restaurantArray.append(value['name']);
            
        recommendation = random.choice(restaurantArray)
